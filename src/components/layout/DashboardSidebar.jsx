...
const Logo = ({ collapsed }) => (
  <Link to="/" className={`flex items-center ${collapsed ? 'justify-center' : 'gap-1'}`}>
    <span className={`text-2xl font-[900] tracking-tighter text-white ${collapsed ? 'hidden' : 'block'}`}>l</span>
    ...
    <span className={`text-2xl font-[900] tracking-tighter text-white ${collapsed ? 'hidden' : 'block'}`}>mo</span>
  </Link>
);
...
const DashboardSidebar = () => {
	...
	return (
		<>
			...
			{/* Sidebar */}
			<aside
				className={`fixed left-0 top-0 h-screen bg-black border-r border-white/5 z-50 transition-all duration-500 ease-[cubic-bezier(0.2,0,0,1)] ${
			...
			>
				<div className="flex flex-col h-full font-['Plus_Jakarta_Sans',sans-serif]">
					...
					<div className="px-4 mb-8">
						<Link to="/dashboard/projects/new">
							<motion.button
								...
								whileTap={{ scale: 0.98 }}
								className={`w-full flex items-center gap-3 p-3.5 bg-yellow-400 text-black font-black uppercase tracking-widest text-[10px] shadow-[4px_4px_20px_0px_rgba(250,204,21,0.2)] hover:bg-white transition-all ${
                    ...
                    {active && (
                      <motion.div 
                        layoutId="nav-glow"
                        className="absolute inset-0 bg-yellow-400/10 border-r-2 border-yellow-400"
                        transition={{ type: "spring", stiffness: 300, damping: 30 }}
                      />
                    )}
										<Icon className={`w-5 h-5 flex-shrink-0 transition-colors z-10 ${
											active ? "text-yellow-400" : "text-gray-500 group-hover:text-white"
										}`} />
										{sidebarOpen && (
					...
					<button
						onClick={toggleSidebar}
						className="hidden lg:flex items-center justify-center h-14 border-t border-white/5 text-gray-600 hover:text-yellow-400 hover:bg-white/5 transition-all group"
					>
						{sidebarOpen ? (
